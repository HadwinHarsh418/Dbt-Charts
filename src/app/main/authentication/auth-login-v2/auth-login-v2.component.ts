...
export class AuthLoginV2Component implements OnInit {
  ...
  onSubmit() {
    ...
    this._authenticationService
      .login(this.f.email.value, this.f.password.value, false, locData)
      .pipe(first())
      .subscribe(
        data => {
          if (!data.error && data.body && data.body && data.body.token) {
            let cUser: User = {
              ...
              avatar: data.body.profile_picture ? data.body.profile_picture : 'assets/images/avatars/10.png',
              profile_picture: data.body.profile_picture,
              wrong_login_attempts: data.body.wrong_login_attempts,
              _id: data.body._id,
              two_fa_actived: data.body.two_fa_actived ? data.body.two_fa_actived : '0',
